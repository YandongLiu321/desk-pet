...
| 功能 | 描述 | 优先级 |
|------|------|--------|
| 透明无边框窗口 | 300×400px，置顶，可拖动 | P0 |
| 角色占位展示 | 纯图标 + CSS 呼吸动画 | P0 |
| 系统托盘 | 托盘图标 + 右键菜单（切换模式、退出） | P0 |
| 对话气泡 | 点击角色弹出气泡，单行输入框 + 回车发送 | P0 |
| AI 对话 | 接入 DeepSeek API，世界书 Prompt 注入 | P0 |
| 主动交互 | 简化版：仅支持手动点击触发对话。完整版含 4 种触发源，见 4.4.2 | P1 |
| 任务发布 | 用户在对话中表达待办，AI 转化为叙事任务 | P0 |
| 迷你任务面板 | 右键菜单展开，查看/勾选子任务 | P0 |
| 模式切换入口 | 右键菜单进入壁纸/软件模式 | P0 |
...
| 项目 | 说明 |
|------|------|
| **原型做法** | 单次倒计时，时间到角色询问一句，用户手动选择"完成/未完成" |
| **完整需求** | 时间到后角色主动询问"完成了/还没/完成了一部分"；若未完成则询问"还需要多少时间？"自动进入下一轮；支持连续番茄钟自动循环 |
| **简化原因** | 完整对话式循环需要多轮 LLM 交互，原型阶段用按钮选择简化 |
| **扩展接口** | `src/main/pomodoro-service.js` |
